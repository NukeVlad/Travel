...
if( ! nv_function_exists( 'nv_block_news_cat' ) )
{
	...
	function nv_block_news_cat( $block_config )
	{
		...
		if( ! empty( $list ) )
		{
			if( file_exists( NV_ROOTDIR . '/themes/' . $global_config['module_theme']  . '/modules/travel/block_groups.tpl' ) )
			{
			...
			$xtpl = new XTemplate( 'block_groups.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/travel' );
			foreach( $list as $l )
			{
