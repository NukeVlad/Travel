...
if( empty( $contents ) )
{
	...
	if( $viewcat == 'viewcat_page_new' or $viewcat == 'viewcat_page_old' or $set_view_page )
	{
		...
		$db->select( 'id, listcatid, topicid, admin_id, author, sourceid, addtime, edittime, publtime, title, alias, hometext,  giatour, matour, tiente, donvitinh, hanhtrinh, khoihanh, ketthuc, thoigian, ghichutour, loaikhachsan, loaiphong, giaphong, tiennghiks, tiennghitp, ghichuks, homeimgfile, homeimgalt, homeimgthumb, allowed_rating, hitstotal, hitscm, total_rating, click_rating' )
			->order( $order_by )
			->limit( $per_page )
	...
	elseif( $viewcat == 'viewcat_main_left' or $viewcat == 'viewcat_main_right' or $viewcat == 'viewcat_main_bottom' )
	{
		...
		if( $global_array_cat[$catid]['subcatid'] != '' )
		{
			...
			foreach( $array_catid as $catid_i )
			{
				...
				$db->sqlreset()
					->select( 'id, catid, listcatid, topicid, admin_id, author, sourceid, addtime, edittime, publtime, title, alias, hometext,  giatour, matour, tiente, donvitinh, hanhtrinh, khoihanh, ketthuc, thoigian, ghichutour, loaikhachsan, loaiphong, giaphong, tiennghiks, tiennghitp, ghichuks, homeimgfile, homeimgalt, homeimgthumb, allowed_rating, hitstotal, hitscm, total_rating, click_rating' )
	...
	elseif( $viewcat == 'viewcat_two_column' )
	{
		...
		$db->sqlreset()
			->select( 'id, listcatid, topicid, admin_id, author, sourceid, addtime, edittime, publtime, title, alias, hometext,  giatour, matour, tiente, donvitinh, hanhtrinh, khoihanh, ketthuc, thoigian, ghichutour, loaikhachsan, loaiphong, giaphong, tiennghiks, tiennghitp, ghichuks, homeimgfile, homeimgalt, homeimgthumb, allowed_rating, hitstotal, hitscm, total_rating, click_rating' )
			->from( NV_PREFIXLANG . '_' . $module_data . '_' . $catid )
			->where( 'status=1' )
			->order( 'publtime DESC' )
			->limit( $per_page )
