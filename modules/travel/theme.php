...
function viewcat_two_column( $array_content, $array_catpage )
{
	...
	foreach( $array_catpage as $key => $array_catpage_i )
	{
		...
		if( $number_content > 0 )
		{
			...
			$newday = $array_content_i['publtime'] + ( 86400 * $array_content_i['newday'] );
			$array_content_i['hometext'] = nv_clean60( $array_content_i['hometext'], 150);
			$array_content_i['publtime'] = nv_date( 'd/m/Y h:i:s A', $array_content_i['publtime'] );
...
function detail_theme( $news_contents, $array_keyword, $related_new_array, $related_array, $topic_array, $content_comment )
{
	...
	// Nieu co thi xuat ra. Nieu khong co thong tin thi an noi dung
	
	if( ! empty( $news_contents['giatour'] ) )
		{
			$xtpl->parse( 'main.giatour' );
		} 
	if( ! empty( $news_contents['hanhtrinh'] ) )
		{
			$xtpl->parse( 'main.hanhtrinh' );
		} 
	if( ! empty( $news_contents['khoihanh'] ) )
		{
			$xtpl->parse( 'main.khoihanh' );
		} 
	if( ! empty( $news_contents['ketthuc'] ) )
		{
			$xtpl->parse( 'main.ketthuc' );
		} 
	if( ! empty( $news_contents['thoigian'] ) )
		{
			$xtpl->parse( 'main.thoigian' );
		} 
	if( ! empty( $news_contents['ghichutour'] ) )
		{
			$xtpl->parse( 'main.ghichutour' );
		} 
		// Het thong tin tour
		
		 
	if( ! empty( $news_contents['loaikhachsan'] ) )
		{
			$xtpl->parse( 'main.loaikhachsan' );
		} 
	if( ! empty( $news_contents['loaiphong'] ) )
		{
			$xtpl->parse( 'main.loaiphong' );
		}
	if( ! empty( $news_contents['giaphong'] ) )
		{
			$xtpl->parse( 'main.giaphong' );
		}
	if( ! empty( $news_contents['tiente'] ) )
		{
			$xtpl->parse( 'main.tiente' );
		}
	if( ! empty( $news_contents['donvitinh'] ) )
		{
			$xtpl->parse( 'main.donvitinh' );
		}
	if( ! empty( $news_contents['tiennghitp'] ) )
		{
			$xtpl->parse( 'main.tiennghitp' );
		}
	if( ! empty( $news_contents['tiennghiks'] ) )
		{
			$xtpl->parse( 'main.tiennghiks' );
		}
	if( ! empty( $news_contents['ghichuks'] ) )
		{
			$xtpl->parse( 'main.ghichuks' );
		}
	// Het phan them vao
...
if( !empty( $content_dattour ) )
	{
		$xtpl->assign( 'CONTENT_DATTOUR', $content_dattour );
		$xtpl->parse( 'main.dattour' );
	}
